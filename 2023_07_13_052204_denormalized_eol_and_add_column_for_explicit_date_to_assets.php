...
class DenormalizedEolAndAddColumnForExplicitDateToAssets extends Migration
{
     ...
     * mysql, postgres, sqlite, or sqlsrv depending on the driver being used.
     */
    private function eolUpdateExpression(): Expression
    {
        ...
        if (DB::getDriverName() === 'sqlsrv') {
            // Correct syntax for SQL Server
            return DB::raw("DATEADD(month, (SELECT eol FROM " . DB::getTablePrefix() . "models WHERE models.id = assets.model_id), purchase_date)");
        }
