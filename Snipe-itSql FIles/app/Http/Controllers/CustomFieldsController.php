    ...
    public function store(CustomFieldRequest $request) : RedirectResponse
    {
        ...
        if ($field->save()) {
            ...
            if ($request->has('associate_fieldsets') && (is_array($fieldset_array))) {
                // Build sync data with required field and order
                $syncData = [];
                $order = 1;
                foreach (array_keys($fieldset_array) as $fieldset_id) {
                    // Get the max order for this fieldset
                    $maxOrder = \DB::table('custom_field_custom_fieldset')
                        ->where('custom_fieldset_id', $fieldset_id)
                        ->max('order');
                    
                    $syncData[$fieldset_id] = [
                        'required' => 0,
                        'order' => $maxOrder ? $maxOrder + 1 : 1
                    ];
                }
                $field->fieldset()->sync($syncData);
            } else {
                $field->fieldset()->sync([]);
            }
    ...
    public function update(CustomFieldRequest $request, CustomField $field) : RedirectResponse
    {
        ...
        if ($field->save()) {
            ...
            if ($request->has('associate_fieldsets') && (is_array($fieldset_array))) {
                ...
                $syncData = [];
                foreach (array_keys($fieldset_array) as $fieldset_id) {
                    // Check if this field is already in this fieldset
                    $existing = \DB::table('custom_field_custom_fieldset')
