   ...
   public static function boot()
{
    ...
    self::created(function ($custom_field) {
        ...
        // Using direct DB update to avoid identity column issue
        \DB::table('custom_fields')
            ->where('id', $custom_field->id)
            ->update([
                'db_column' => $custom_field->convertUnicodeDbSlug(),
                'updated_at' => now()
            ]);
    });
    ...
    self::updating(function ($custom_field) {
        ...
        if ($custom_field->isDirty('name')) {
            ...
            // Just set the db_column - don't call save()
    ...
    self::deleting(function ($custom_field) {
        // Only try to drop the column if db_column is not empty
