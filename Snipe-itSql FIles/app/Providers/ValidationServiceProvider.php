...
class ValidationServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        /**
         * MSSQL datetime validator that handles milliseconds
         * This validator accepts datetime values with or without milliseconds
         */
        Validator::extend('mssql_datetime', function ($attribute, $value, $parameters, $validator) {
            if (empty($value)) {
                return true; // nullable fields pass when empty
            }
            
            // Try to parse the date with various formats
            $formats = [
                'Y-m-d H:i:s.v',     // With milliseconds (MSSQL format)
                'Y-m-d H:i:s.u',     // With microseconds
                'Y-m-d H:i:s',       // Standard format
                'Y-m-d\TH:i:s.v',    // ISO format with milliseconds
        ...
        /**
         * MSSQL date validator (without time)
         */
        Validator::extend('mssql_date', function ($attribute, $value, $parameters, $validator) {
            if (empty($value)) {
                return true; // nullable fields pass when empty
            }
        ...
        Validator::extend('unique_undeleted', function ($attribute, $value, $parameters, $validator) {
            if (count($parameters)) {
                // Serial number special case
                if (($parameters[0]=='assets') && ($attribute == 'serial') && (Setting::getSettings()->unique_serial != '1')) {
                    return true;
                }

                $table = $parameters[0];
                $column = $attribute; // Use the attribute name as column
                $excludeId = isset($parameters[1]) && is_numeric($parameters[1]) ? $parameters[1] : 0;
        ...
        /**
         * Add custom validation messages for MSSQL validators
         */
        Validator::replacer('mssql_datetime', function($message, $attribute, $rule, $parameters) {
            return str_replace(':attribute', $attribute, 'The :attribute must be a valid date and time.');
        });

        Validator::replacer('mssql_date', function($message, $attribute, $rule, $parameters) {
            return str_replace(':attribute', $attribute, 'The :attribute must be a valid date.');
        });
    }
