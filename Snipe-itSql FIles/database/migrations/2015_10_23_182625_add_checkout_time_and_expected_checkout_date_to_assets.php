    ...
    public function up()
{
    ...
    DB::statement('UPDATE '.DB::getTablePrefix().'assets SET expected_checkin=(SELECT expected_checkin FROM (SELECT expected_checkin, ROW_NUMBER() OVER (ORDER BY id DESC) as row FROM '.DB::getTablePrefix().'asset_logs WHERE '.DB::getTablePrefix().'asset_logs.id='.DB::getTablePrefix()."assets.id AND action_type='checkout') as tempTable WHERE row = 1) WHERE assigned_to IS NOT NULL");
}
